...
- Windows 一键包仍不含手机 APK。`android/` 已按 v1.5.2 做成独立安卓应用：不遥控电脑，只填 NovelAI 和 DeepSeek 即可在线换角出图。

## v1.5.1 相对 v1.5.0

- 一级导航收到 6 项，设置进顶栏；待生成 / 爬虫 / 分类进「更多」，页面仍在。
- 工作台可一次入队本系列全部页；重启取消排队后，工作台和生成库可以续跑未发出的页。
- 生成库按源作品收成一套封面；删除进回收站，可恢复，不会自动清空。
- 工作台、在线换角、换角插件都锁 Opus 免费档。
- 后处理任务可以取消。
- NovelAI Token 以设置中心账号池为准；换角页仍可展开高级编辑，改的是同一份池子。
- Windows 一键包不含手机 APK。`android/` 与 `/m` 仍在源码里，不作为本版交付。
