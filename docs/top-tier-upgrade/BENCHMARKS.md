...
| Check | Result | Notes |
|---|---|---|
| Critical paid/butler/generation/gallery subset | recorded after this run | no Windows shell |
| `tests/tooling` + NAI compile lock | recorded after this change | no paid API |
| `scripts/bench_gallery.py --count 1000 --repeats 20` | synthetic in-memory | not a 10k/100k claim |
