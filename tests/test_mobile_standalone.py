...
class MobileStandaloneTests(unittest.TestCase):
    ...
    def test_web_shell_has_standalone_mode(self) -> None:
        ...
        self.assertIn("写 OC", js)
        self.assertIn("searchSource", js)
        ...
        self.assertIn("正在抓咒语", js)
        self.assertIn("workHasPrompts", js)
        self.assertIn("写 OC", js)
        self.assertIn("整段咒语", js)
        self.assertIn("群友 OC", js)
        self.assertIn("{{权重}}", js)
        self.assertIn("服饰替换", js)
        self.assertNotIn("高级：只换本页", js)
        self.assertIn('id="mApplyOne"', js)
        self.assertIn("本页换角", js)
        self.assertIn("全部页按槽位换", js)
        self.assertIn("全部页加入队列", js)
        self.assertIn("data-del-job", js)
        self.assertIn("清空已结束", js)
        self.assertIn("生成连接被掐断", js)
        ...
        self.assertIn("slot_targets", core)
        self.assertIn("countGenderSlots", core)
        ...
        self.assertIn("request.query(\"source\")", server)
        custom = (ANDROID / "app/src/main/java/com/naixuezhang/studio/mobile/CustomCharStore.java").read_text(
            encoding="utf-8"
        )
        self.assertIn("oc_mode", custom)
        self.assertIn("整段角色咒语", custom)
        ...
        self.assertIn("Connection", http)
        self.assertIn("isTransport", http)
        ...
        self.assertIn("normalizeSource", chars)
